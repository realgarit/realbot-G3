...
# Here's the list of what pip needs to install, just like a requirements.txt file.
required_modules = [
    "confz==2.0.1",
    "numpy~=2.1.0",
    "setuptools",
...
def update_requirements(ask_for_confirmation: bool = True) -> bool:
    ...
    # Some stuff only works on 64-bit Python. It's not the 90s, so we're just making that a requirement.
    if platform.architecture()[0] != "64bit":
        print(f"ERROR: A 64-bit version of Python is required in order to run {realbot_name} {realbot_version}.\n")
        print(f"You are currently running a {platform.architecture()[0]} version.")
        sys.exit(1)

    # We don't want to mess up your global Python setup without asking.
    # But if you're in a virtual environment, we'll just go ahead and skip the check.
    ...
    # Time to get pip to install everything.
    import subprocess
    ...
    # Now let's make sure libmgba-py is ready.
    print("")
    ...
    # We'll mark these as checked so we don't have to do it every single time you start the bot.
...
def check_requirements() -> bool:
    ...
    # If this is a standalone app from PyInstaller, we don't need to check this.
    # Everything is already packed in, and we can't download more anyway.
    if is_bundled_app():
        return True

    # Checking for updates on every start is annoying.
    # We'll save the version in .last-requirements-check.
    # If it's already there and nothing changed, we'll just move on.
    ...
    if requirements_file.is_file():
        with open(requirements_file, "r") as file:
            if file.read() != requirements_hash:
                print(
                    f"This is a newer version of {realbot_name} than you have run before, "
        ...
        print(
            f"Seems like this is the first time you are running {realbot_name}!\n"
